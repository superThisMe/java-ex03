...
public class BaseballGame2 {
	...
	public void startGame() {
		...
		for (int i = 0; i < cnumbers.length; i++) { // i는 3까지 가는 반복
			cnumbers[i] = (int)(Math.random() * 10); // 0 ~ 9
			for (int j = 0; j < i; j++) { // j는 i까지 가는 반복
		...
		for (; round < 10; round++) { // 사용자에게 주어지는 10번의 기회
			...
			input_loop: while (true) {
				...
				for (int i = 0; i < unumbers.length; i++) {
					...
					for (int j = 0; j < i; j++) {
						if (unumbers[i] == unumbers[j]) { // 중복이 발생한 경우
							continue input_loop; //input_loop 반복문의 처음으로 이동
						}
					}
				}
